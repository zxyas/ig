...
def login_dev():
	...
	username_dev = input("\n [?] username instagram : ")
	pass_dev = input(" [?] password instagram : ")
	try:
		try:
			headerz = {"User-Agent": user_agentz}
...
def menu_dev():
	...
	if pil == "1" or pil == "01":
		pas = ""
		status = ""
		username = input(" [?] masukan username target : ")
		...
		usr_ = input(" [?] masukan nama pencarian (cth : yoga): ")
		...
		kel = input(" [?] yakin mau keluar dari akun Instagram?[Y/t] : ")
		if kel in ["y", "Y"]:
			hapus_cookie()
